<nav class="sb-topnav navbar navbar-expand navbar-dark bg-primary">
            <!-- Navbar Brand-->
            <a class="navbar-brand  ps-3" href="#">Mercal C.A</a>     
            <!-- Navbar Search-->
             <a class="navbar-brand  ps-2" href="datos_user.php">Usuarios</a>    
             <a class="navbar-brand  ps-3" href="admin.php">Inicio</a>   
            <form class="d-none d-md-inline-block form-inline ms-auto me-0 me-md-3 my-2 my-md-0">
                <div class="input-group">
                    <input class="form-control" type="hidden" placeholder="Search for..." aria-label="Search for..." aria-describedby="btnNavbarSearch" />
                   
                </div>
            </form>
            <!-- Navbar-->
             <a style="color:white;  text-decoration: none;" href="">administrador</a>
            <ul class="navbar-nav ms-auto ms-md-0 me-3 me-lg-4">
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" id="navbarDropdown" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false"><img src="img/perf.png" style="border-radius :200px; height: 50px;" alt=""></i></a>
                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                       
                        <li><a class="dropdown-item" href="config/logout.php">Salir</a></li>
                    </ul>
                </li>
            </ul>
</nav>